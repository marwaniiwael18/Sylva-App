...
use App\Models\TreeMaintenance;
use App\Models\Tree;
...
{
    /**
     * Display a listing of tree maintenance records.
     * Can filter by tree_id, user_id, event_id, activity_type
     */
    public function index(Request $request): JsonResponse
    {
        $query = TreeMaintenance::with(['tree', 'maintainer', 'event'])
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $maintenance = TreeMaintenance::create($data);
    ...
    {
        $maintenance = TreeMaintenance::with(['tree', 'maintainer', 'event'])->find($id);
    ...
    {
        $maintenance = TreeMaintenance::find($id);
    ...
    public function stats(Request $request): JsonResponse
    {
        ...
        $stats = [
            ...
            'recent_activities' => TreeMaintenance::with(['tree', 'maintainer'])
    ...
    public function treeHistory(string $treeId): JsonResponse
    {
        ...
        $maintenances = TreeMaintenance::with(['maintainer', 'event'])
            ->byTree($treeId)
            ->orderBy('performed_at', 'desc')
            ->get();
    ...
    public function userActivities(Request $request): JsonResponse
    {
        ...
        $maintenances = TreeMaintenance::with(['tree', 'event'])
            ->byUser($userId)
            ->orderBy('performed_at', 'desc')
            ->paginate(20);

        $stats = [
            'total_activities' => TreeMaintenance::byUser($userId)->count(),
            'trees_maintained' => TreeMaintenance::byUser($userId)->distinct('tree_id')->count('tree_id'),
            'this_month' => TreeMaintenance::byUser($userId)->thisMonth()->count(),
            'favorite_activity' => TreeMaintenance::byUser($userId)->get()->groupBy('activity_type')->sortByDesc->count()->keys()->first()
        ];
